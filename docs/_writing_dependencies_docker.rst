...
BioContainers it will attempt to build these containers on-demand by default
...
Building unpublished BioContainers on-demand is great for testing but
...
using these long mulled hashes. The Planemo command ``container_register``
will inspect a tool and open a Github pull request to add the tool's combination
of packages to the registry resulting. Upon merging, this pull request will
...
``container_register`` command. To simplify all of this - we maintain a list
of Github repositories containing Galaxy and/or CWL tools that we scan daily
and create hashes for using ``container_register``. This list is maintained
in a script named ``monitor.sh`` in the `planemo-monitor
<https://github.com/galaxyproject/planemo-monitor/>`__ repository. Simply open
a pull request to add your tool repositories to this list and we will monitor
all your tools as well.
