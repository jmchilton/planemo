...
class BaseEngine(Engine):
    ...
    def _run_test_cases(self, test_cases, test_timeout):
        ...
        job_paths = []
        output_collectors = []
        with tempfile.TemporaryDirectory(prefix="planemo-test-jobs-") as test_job_directory:
            for test_index, test_case in enumerate(test_cases):
                if test_case.job_path is None:
                    job_path = os.path.join(test_job_directory, f"job-{test_index}.json")
                    job = _absolutize_job_paths(test_case.job, test_case.tests_directory)
